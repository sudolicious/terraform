...
CONTROL_IP=$(grep -A1 '\[kube_control_plane\]' inventory/mycluster/inventory.ini | tail -n1 | awk '{print $2}' | cut -d'=' -f2)
echo "Control plane IP: $CONTROL_IP"

# Copy kube config 
echo "Copying kubeconfig to Windows..."
mkdir -p /mnt/c/Users/olga/.kube
...
echo "Kubespray setup completed successfully!"